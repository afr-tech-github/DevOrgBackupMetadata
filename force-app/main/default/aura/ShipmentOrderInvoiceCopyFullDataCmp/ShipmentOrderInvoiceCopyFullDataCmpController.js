({
    ...
    handleCopyData: function(component, event, helper) {
        var action = component.get('c.cloneFullShipment_lightning');
        component.set("v.isCopying", true);
        action.setParams({
            "sourceShipmentId": component.get("v.recordId")
        });
        action.setCallback(this, function(response) {
            var state = response.getState();
            var idRecord = response.getReturnValue();
            
            // console.log("handleCopyData: ", response.getReturnValue());
            // var res = JSON.parse(response.getReturnValue() || "{}");
            // helper.insertBillings(component, res.MeiList || []);
            // var idRecord = res.clonedShipmentId;

            component.set("v.isCopying", false);
            if (component.isValid() && state === "SUCCESS" && idRecord != '') {
                var message;
